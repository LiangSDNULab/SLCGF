...
def load_cancer_data(CancerName):
   # filePath = path + '/' + fileName
    RNASeq_dataframe = pd.read_csv('/home/lcheng/JiangHuiYing/randomtest/cancerdata/'+CancerName+'/'+CancerName+'exp.csv')
    RNASeq_names = RNASeq_dataframe.iloc[:, 0].tolist()
    RNASeq_dataframe = RNASeq_dataframe.iloc[:, 1:]  # 去掉第一列
    miRNA_dataframe = pd.read_csv('/home/lcheng/JiangHuiYing/randomtest/cancerdata/'+CancerName+'/'+CancerName+'mirna.csv')
    miRNA_dataframe = miRNA_dataframe.iloc[:, 1:]
    clinical_dataframe = pd.read_csv('/home/lcheng/JiangHuiYing/randomtest/cancerdata/'+CancerName+'/'+CancerName+'_survival.csv')
